...
from torch_utils.engine import (
    train_one_epoch, evaluate, evaluate_loss1
)
...
from utils.general import (
    ...
    save_model, save_train_loss_plot, save_train_valid_loss_plot,
...
from utils.logging import (
    ...
    csv_log,
    wandb_log,
    csv_log_loss #Bổ sung ngày 25/03/2023
)
...
def main(args):
    ...
    currentDateAndTime = datetime.now(hcmTz)     
    print("Ngay gio bat dau = " , currentDateAndTime)
    ...
    print(model)
    model= torch.nn.DataParallel(model) #Bo sung de su dung cac GPU co san
    ...
    criterion = torch.nn.MSELoss() #Thêm mới

    for epoch in range(start_epochs, NUM_EPOCHS):
        train_loss_hist.reset()
        val_loss_hist.reset()#moi them
        ...
        #Evalute cho phần loss
        #val_loss = evaluate_loss(model, valid_loader, device=DEVICE)
         
        # update the learning rate
         
        val_loss = evaluate_loss1(model, valid_loader, val_loss_hist, device=DEVICE)
        #print(val_loss)
        ...
        #         images = list(img.to(DEVICE) for img in inputs)
        ...
        #     f'Training Loss: {batch_loss_list:.4f}, '
        #     f'Validation Loss: {val_loss:.4f}, '
        #   )
 

        val_loss_list_epoch.append(val_loss_hist.value) #Nối vào
        ...
        #25/03/2023
        #Thêm vào để xuất ra hình training loss và validate loss 25/03/2023
        save_train_valid_loss_plot(
            OUT_DIR, 
            train_loss_list_epoch,
            val_loss_list_epoch, #Bo sung them
            'epochs',
            'loss',
            save_name='loss_epoch' 
        )
        #Ghi log lại 
        csv_log_loss(OUT_DIR, val_loss_list_epoch, train_loss_list_epoch, epoch)


        #Save file log cho phần validation loss theo eposh
        tensorboard_loss_log('Validation loss', np.array(val_loss_list_epoch), writer)
    ...
    currentDateAndTime_last = datetime.now(hcmTz)
    print("Ngay ket thuc = " ,currentDateAndTime_last)
    print("Ngay ket thuc cuoi cung = " , currentDateAndTime_last - currentDateAndTime)
